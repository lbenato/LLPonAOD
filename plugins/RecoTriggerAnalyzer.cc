...
RecoTriggerAnalyzer::RecoTriggerAnalyzer(edm::ParameterSet& PSet, edm::ConsumesCollector&& CColl):
    ...
    //L1FiltersList(PSet.getParameter<std::vector<std::string> >("l1filters"))//not in aod
{
    std::cout << " --- RecoTriggerAnalyzer initialization ---" << std::endl;
...
RecoTriggerAnalyzer::~RecoTriggerAnalyzer() {
...
void RecoTriggerAnalyzer::FillTriggerMap(const edm::Event& iEvent, std::map<std::string, bool>& Map) {
    ...
    for(unsigned int i = 0; i < TriggerList.size(); i++) {
        ...
        for(unsigned int j=0, in=trigNames.size(); j < in; j++) {
            if(trigNames.triggerName(j).find(TriggerList[i]) != std::string::npos) {
                ...
                //PrescalesMap[TriggerList[i]] = triggerPrescales->getPrescaleForIndex(index); //they must be filled even if the trigger is not fired!!!!//not sure if in AOD TODO
...
/*
std::vector<pat::TriggerObjectStandAlone> RecoTriggerAnalyzer::FillTriggerObjectVector(const edm::Event& iEvent, std::string& TrigName) {
//std::vector<pat::TriggerObjectStandAlone> RecoTriggerAnalyzer::FillTriggerObjectVector(const edm::Event& iEvent) {
...
void RecoTriggerAnalyzer::FillMetFiltersMap(const edm::Event& iEvent, std::map<std::string, bool>& Map) { //, edm::EDGetTokenT<edm::TriggerResults>& iToken, std::vector<std::string>& iList) {
...
////////////////////////////////////////////////
/*
void RecoTriggerAnalyzer::FillL1FiltersMap(const edm::Event& iEvent, std::map<std::string, bool>& Map) {
...
bool RecoTriggerAnalyzer::GetBadPFMuonFlag(const edm::Event& iEvent) { 
...
bool RecoTriggerAnalyzer::GetBadChCandFlag(const edm::Event& iEvent) { 
...
//Pre-Firing
bool RecoTriggerAnalyzer::EvaluatePrefiring(const edm::Event& iEvent) { 
...
void RecoTriggerAnalyzer::L1Bits(const edm::Event& iEvent) {
...
/*
void RecoTriggerAnalyzer::Debug(const edm::Event& iEvent) {

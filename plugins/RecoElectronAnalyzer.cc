...
RecoElectronAnalyzer::RecoElectronAnalyzer(const edm::ParameterSet& PSet, edm::ConsumesCollector&& CColl):
    ElectronToken(CColl.consumes<std::vector<reco::GsfElectron> >(PSet.getParameter<edm::InputTag>("electrons"))),
    ...
    Electron2Pt(PSet.getParameter<double>("electron2pt"))
{
    ...
    if(!EleSingleTriggerIsoFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No SingleTriggerIso File");
      return;
    }
    ...
    if(!EleSingleTriggerFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No SingleTrigger File");
      return;
    }
    ...
    if(!EleRecoEffFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleRecoEff Weight File");
      return;
    }
    ...
    if(!EleVetoIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleVetoId Weight File");
      return;
    }
    ...
    if(!EleLooseIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleLooseId Weight File");
      return;
    }
    ...
    if(!EleMediumIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleMediumId Weight File");
      return;
    }
    ...
    if(!EleTightIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleTightId Weight File");
      return;
    }
    ...
    if(!EleMVATrigMediumIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleMVATrigMediumId Weight File");
      return;
    }
    ...
    if(!EleMVATrigTightIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoElectronAnalyzer", "No EleMVATrigTightId Weight File");
      return;
    }
    
    eScaleSmearer = new EnergyScaleCorrection_class(EleScaleSmearCorrectionName);

    std::cout << " --- RecoElectronAnalyzer initialization ---" << std::endl;
...
RecoElectronAnalyzer::~RecoElectronAnalyzer() {
...
std::vector<reco::GsfElectron> RecoElectronAnalyzer::FillElectronVector(const edm::Event& iEvent) {
    bool isMC(!iEvent.isRealData());
    int IdTh(Electron1Id);//, IsoTh(Electron1Iso);
    float PtTh(Electron1Pt);
    std::vector<reco::GsfElectron> Vect;
    ...
    //edm::Handle<std::vector<reco::Conversion> > EleConv;
    ...
    for(std::vector<reco::GsfElectron>::const_iterator it=EleCollection->begin(); it!=EleCollection->end(); ++it) {
        if(Vect.size()>0) {
            IdTh=Electron2Id;
            //IsoTh=Electron2Iso;
            PtTh=Electron2Pt;
        }
        reco::GsfElectron el=*it;
        reco::GsfElectronRef elRef(EleCollection, elIdx);
...
/*
void RecoElectronAnalyzer::AddVariables(std::vector<reco::GsfElectron>& Vect, pat::MET& MET) {
...
float RecoElectronAnalyzer::GetElectronIdSF(reco::GsfElectron& el, int id) {
...
float RecoElectronAnalyzer::GetElectronIdSFError(reco::GsfElectron& el, int id) {
    if(id==0 && isEleVetoIdFile){
        double pt = std::min( std::max( ElectronIdVeto->GetYaxis()->GetXmin(), el.pt() ) , ElectronIdVeto->GetYaxis()->GetXmax() - 0.000001 );
        double abseta = std::min( ElectronIdVeto->GetXaxis()->GetXmax() - 0.000001 , fabs(el.eta()) );
...
float RecoElectronAnalyzer::GetDoubleElectronTriggerSF(reco::GsfElectron& el1, reco::GsfElectron& el2) { //obsolete!
...
float RecoElectronAnalyzer::GetElectronRecoEffSF(reco::GsfElectron& el) {
...
float RecoElectronAnalyzer::GetElectronRecoEffSFError(reco::GsfElectron& el) {
    if(!isEleRecoEffFile) return 1.;
    double pt = std::min( std::max( ElectronRecoEff->GetYaxis()->GetXmin(), el.pt() ) , ElectronRecoEff->GetYaxis()->GetXmax() - 0.000001 );
    double abseta = std::min( ElectronRecoEff->GetXaxis()->GetXmax() - 0.000001 , fabs(el.eta()) );
...
float RecoElectronAnalyzer::GetElectronTriggerSFEle105(reco::GsfElectron& ele) {
...
float RecoElectronAnalyzer::GetElectronTriggerSFErrorEle105(reco::GsfElectron& ele) {
    if(!isEleSingleTriggerFile) return 1.;
    double pt = std::min( std::max( ElectronTriggerEle105->GetYaxis()->GetXmin(), ele.pt() ) , ElectronTriggerEle105->GetYaxis()->GetXmax() - 0.000001 );
    double eta = 0.;
    if (ele.eta() > 0)
...
float RecoElectronAnalyzer::GetElectronTriggerSFEle27Tight(reco::GsfElectron& ele) {
...
float RecoElectronAnalyzer::GetElectronTriggerSFErrorEle27Tight(reco::GsfElectron& ele) {
  if(!isEleSingleTriggerIsoFile) return 1.;
  double pt = std::min( std::max( ElectronTriggerEle27Tight->GetYaxis()->GetXmin(), ele.pt() ) , ElectronTriggerEle27Tight->GetYaxis()->GetXmax() - 0.000001 );
  double eta = 0.;
  if (ele.eta() > 0)

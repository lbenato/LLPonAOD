...
RecoPhotonAnalyzer::RecoPhotonAnalyzer(edm::ParameterSet& PSet, edm::ConsumesCollector&& CColl):
    PhotonToken(CColl.consumes<std::vector<reco::Photon> >(PSet.getParameter<edm::InputTag>("photons"))),
...

{
    ...
    if(!PhoLooseIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoPhotonAnalyzer", "No PhoLooseId Weight File");
      return;
    }
    ...
    if(!PhoMediumIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoPhotonAnalyzer", "No PhoMediumId Weight File");
      return;
    }
    ...
    if(!PhoTightIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoPhotonAnalyzer", "No PhoTightId Weight File");
      return;
    }
    ...
    if(!PhoMVANonTrigMediumIdFile->IsZombie()) {
      ...
      throw cms::Exception("RecoPhotonAnalyzer", "No PhoMVANonTrigMediumId Weight File");
      return;
    }
    
    
    std::cout << " --- RecoPhotonAnalyzer initialization ---" << std::endl;
    std::cout << "  photon pT         :\t" << PhotonPt << std::endl;
    std::cout << "  photon id         :\t" << PhotonId << std::endl;
    std::cout << std::endl;
}

RecoPhotonAnalyzer::~RecoPhotonAnalyzer() {
...
std::vector<reco::Photon> RecoPhotonAnalyzer::FillPhotonVector(const edm::Event& iEvent) {
    bool isMC(!iEvent.isRealData());
    int IdTh(PhotonId);
    float PtTh(PhotonPt);
    std::vector<reco::Photon> Vect;
    // Declare and open collection
    edm::Handle<std::vector<reco::Photon> > PhoCollection;
    ...
    for(std::vector<reco::Photon>::const_iterator it=PhoCollection->begin(); it!=PhoCollection->end(); ++it) {
        reco::Photon ph=*it;
  reco::PhotonRef phRef(PhoCollection,phIdx);
        ...
        //float pfIso = ( ph.chargedHadronIso() + std::max(ph.neutralHadronIso() + ph.photonIso() - 0.5*ph.puChargedHadronIso(), 0.) ) / ph.pt();//not used in reco
...
void RecoPhotonAnalyzer::PlotPhotons(std::vector<reco::Photon>& PhotonVect, std::map<std::string, TH1F*>& Hist, float EventWeight) {
...
float RecoPhotonAnalyzer::GetPhotonIdSFLoose(reco::Photon& el) {
...
float RecoPhotonAnalyzer::GetPhotonIdSFLooseError(reco::Photon& el) {
  if(!isPhoLooseIdFile) return 1.;
  double pt = std::min( std::max( PhotonIdLoose->GetYaxis()->GetXmin(), el.pt() ) , PhotonIdLoose->GetYaxis()->GetXmax() - 0.000001 );
  double abseta = std::min( PhotonIdLoose->GetXaxis()->GetXmax() - 0.000001 , fabs(el.eta()) );
...
float RecoPhotonAnalyzer::GetPhotonIdSFMedium(reco::Photon& el) {
...
float RecoPhotonAnalyzer::GetPhotonIdSFMediumError(reco::Photon& el) {
  if(!isPhoMediumIdFile) return 1.;
  double pt = std::min( std::max( PhotonIdMedium->GetYaxis()->GetXmin(), el.pt() ) , PhotonIdMedium->GetYaxis()->GetXmax() - 0.000001 );
  double abseta = std::min( PhotonIdMedium->GetXaxis()->GetXmax() - 0.000001 , fabs(el.eta()) );
...
float RecoPhotonAnalyzer::GetPhotonIdSFTight(reco::Photon& el) {
...
float RecoPhotonAnalyzer::GetPhotonIdSFTightError(reco::Photon& el) {
  if(!isPhoTightIdFile) return 1.;
  double pt = std::min( std::max( PhotonIdTight->GetYaxis()->GetXmin(), el.pt() ) , PhotonIdTight->GetYaxis()->GetXmax() - 0.000001 );
  double abseta = std::min( PhotonIdTight->GetXaxis()->GetXmax() - 0.000001 , fabs(el.eta()) );
...
float RecoPhotonAnalyzer::GetPhotonIdSFMVANonTrigMedium(reco::Photon& el) {
...
float RecoPhotonAnalyzer::GetPhotonIdSFMVANonTrigMediumError(reco::Photon& el) {
  if(!isPhoMVANonTrigMediumIdFile) return 1.;
  double pt = std::min( std::max( PhotonIdMVANonTrigMedium->GetYaxis()->GetXmin(), el.pt() ) , PhotonIdMVANonTrigMedium->GetYaxis()->GetXmax() - 0.000001 );
  double abseta = std::min( PhotonIdMVANonTrigMedium->GetXaxis()->GetXmax() - 0.000001 , fabs(el.eta()) );
...
void RecoPhotonAnalyzer::CleanPhotonsFromMuons(std::vector<reco::Photon>& Photons, std::vector<pat::Muon>& Muons, float angle) {
...
void RecoPhotonAnalyzer::CleanPhotonsFromElectrons(std::vector<reco::Photon>& Photons, std::vector<pat::Electron>& Electrons, float angle) {
...
/*bool RecoPhotonAnalyzer::isLoosePhoton(reco::Photon& el, const reco::Vertex* vertex) {
    return true;
}*/

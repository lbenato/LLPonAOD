...
void ObjectsFormat::FillJetType(JetType& I, const reco::Jet* R, bool isMC) {
  if(!R) return;
  I.pt          = R->pt();
  I.eta         = R->eta();
  I.phi         = R->phi();
...
std::string ObjectsFormat::ListJetType() {return "pt/F:eta/F:phi/F:mass/F";}
...
void ObjectsFormat::FillMEtType(MEtType& I, const reco::PFMET* R, bool isMC) {
    I.pt          = R->pt();
    I.eta         = R->eta();
    I.phi         = R->phi();
...
std::string ObjectsFormat::ListMEtType() {return "pt/F:eta/F:phi/F:sign/F:ptCalo/F";}
...
void ObjectsFormat::FillCandidateType(CandidateType& I, reco::CompositeCandidate* R, bool isMC) {
  if(!R) return;
